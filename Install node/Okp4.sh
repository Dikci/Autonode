...
sed -i.bak -e "s|^proxy_app = \\\"tcp://127.0.0.1:26658\\\"|proxy_app = \\\"tcp://127.0.0.1:$PROXY_APP_PORT\\\"|; s|^laddr = \\\"tcp://127.0.0.1:26657\\\"|laddr = \\\"tcp://127.0.0.1:$LADDR_PORT\\\"|; s|^pprof_laddr = \\\"localhost:6060\\\"|pprof_laddr = \\\"localhost:$LADDR_P2P_PORT\\\"|; s|^laddr = \\\"tcp://0.0.0.0:26656\\\"|laddr = \\\"tcp://0.0.0.0:$PPROF_LADDR_PORT\\\"|; s|^prometheus_listen_addr = \\\":26660\\\"|prometheus_listen_addr = \\\":$PROMETHEUS_PORT\\\"|" $HOME/.okp4d/config/config.toml && sed -i.bak -e "s|^address = \\\"localhost:9090\\\"|address = \\\"localhost:$GRPC_PORT\\\"|; s|^address = \\\"localhost:9091\\\"|address = \\\"localhost:$GRPC_WEB_PORT\\\"|; s|^address = \\\"tcp://localhost:1317\\\"|address = \\\"tcp://localhost:$API_PORT\\\"|" $HOME/.okp4d/config/app.toml
